        ...
        for j in range(1, 4000):
            for t in range(1, 4000):
                for r in range(1, 4000):
                    # print(f'working with parts: {i},{j},{t},{r}')
                    parts = ['x='+str(i),'m='+str(j),'a='+str(t),'s='+str(r)]
                    status = process_part(parts, starting_work_id, work)
                    # print(f'At id: {i},{j},{t},{r} result is {status}', end = '\r')
                    # time.sleep(0.0001)
                    if status == 'A':
                        result += 1
                        if sum_a == 0:
                        ...
                        if sum_r == 0:
                            print(f'At combination {i},{j},{t},{i} \t A: {sum_a}')
                        sum_r += 1
                        sum_a = 0
...
def sol(work):
    ...
    starting_work_id = 'in'
    process_block(block, starting_work_id, work)

def process_block(block, work_id, work):
    workflow = work[work_id]
    # sum = 0 
    for step in workflow:
        # print(f'- working parts {parts} in workflow "{work_id}" step {step} ')
...
# At combination 1,1,1748,1958 s A R: 1231
# At combination 1,1,1748,1 t is R A: 889
# At combination 1,1,1749,1 s As R R: 1153
# At combination 1,1,1749,1  is R  A: 726
# At combination 1,1,1749,1958 s A R: 1231
# At combination 1,1,1749,1 t is R A: 889